function A = gen_nn_distance_output(data, num_neighbors, block_size, save_type, shape_dist)
...
if (euclidean)
    ...
    for i = 1:num_iter
      ...
      dist = aa + bb - 2*ab;
      clear bb ab block;
      dist = max(dist, 0);   % Discard some distance close to zero
...
if (chi_square)
   ...
   for i = 1:num_iter
      ...
      % Compute Chi_Squre distance between block and data
      for i = 1:length(data)

          color_hists = [color_hists; double(data(i).color')];  
          if (size(data(i).dense,2) ~= 1)
              data(i).dense = reshape(data(i).dense, 3000, 1); 
          end
          dense_hists = [dense_hists; double(data(i).dense')]; 

      end
      dense_dist = slmetric_pw(dense_hists', dense_hists', 'chisq');
      color_dist = slmetric_pw(color_hists', color_hists', 'chisq'); 
      dist = dense_dist + color_dist + shape_dist; 
